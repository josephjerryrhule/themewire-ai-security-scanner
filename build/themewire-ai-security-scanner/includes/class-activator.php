...
class Themewire_Security_Activator
{
    ...
    public static function activate()
    {
        ...
        try {
            ...
            // Test database connection for Docker/DevKinsta compatibility
            $db_test = $database->test_database_connection();
            if (!$db_test['success']) {
                error_log('TWSS Activation: Database connection test failed - ' . $db_test['message']);
            } else {
                error_log('TWSS Activation: Database connection successful');
            }
        } catch (Exception $e) {
            error_log('TWSS Activation: Database initialization failed - ' . $e->getMessage());
